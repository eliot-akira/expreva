...
export class PrefixParselet<N, T extends Token> implements Parselet<N, T> {
  constructor(public readonly parse: (parser: ExpressionParserInterface<N, T>, token: T) => N) {}
}
...
export class XfixParselet<N, T extends Token> implements Parselet<N, T> {
  constructor(
    public readonly parse: (parser: ExpressionParserInterface<N, T>, token: T, left: N) => N,
...
export class PostfixUnaryParselet<N, T extends Token> extends XfixParselet<N, T> {
  constructor(
    ...
    super(
      (_: ExpressionParserInterface<N, T>, token: T, left: N) => cons.call(this, token, left),
      precedence,
    )
  }
}
